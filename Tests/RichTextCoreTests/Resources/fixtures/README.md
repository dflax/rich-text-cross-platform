...
under its own test directory) is a **real second copy** of this directory, not a symlink — a
symlink here was tried 2026-09-16 and didn't survive SPM's resource-copy step on a fresh CI
checkout (every fixture lookup came back `.notFound` in the built test bundle, even though it
worked on a local machine that already had both paths resolved on disk). Add a fixture to
**both** copies, or CI's `diff -rq fixtures Tests/RichTextCoreTests/Resources/fixtures` step in
